...
def test_serialize_output_keeps_openai_tool_calls_when_reasoning_is_hoisted():
    output = [
        {
            "type": "reasoning",
            "status": "completed",
            "content": [{"type": "output_text", "text": "Plan first"}],
        },
        {
            "type": "web_search_call",
            "id": "ws_123",
            "status": "completed",
            "action": {"type": "search", "queries": ["Open WebUI"]},
        },
        {
            "type": "message",
    ...
    assert rendered.count('type="reasoning"') == 1
    assert 'id="ws_123"' in rendered
    assert "Web Search" in rendered
    assert rendered.index("Plan first") < rendered.index("Final answer")
